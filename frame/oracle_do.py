...
def get_report(tags,url,user,password,report_type,begin_snap,end_snap):
    ...
    if report_type == 'addm':
        ...
        suffix = 'html'
    if report_type =='ash':
        begin_time = datetime.strptime(begin_snap, '%Y-%m-%dT%H:%M').strftime("%Y%m%d-%H%M")
        end_time = datetime.strptime(end_snap, '%Y-%m-%dT%H:%M').strftime("%Y%m%d-%H%M")
...
def get_ash(url,user,password,dbid,instance_num,begin_snap,end_snap):
    ...
    cur = dbconn.cursor()
    start_time = datetime.strptime(begin_snap, '%Y-%m-%dT%H:%M')
    end_time = datetime.strptime(end_snap, '%Y-%m-%dT%H:%M')
