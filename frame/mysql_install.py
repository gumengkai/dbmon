...
# 上传文件
def sftp_upload_file(host,user,password,local_path, remote_path):
    try:
...
def generate_conf(host,user,password, port, data_path, mysql_base, mysql_home, extra_params):
    """
...
def mysql_install(host,user,password,data_path,mysql_base,port):
    ...
    generate_conf(host,user,password,port,data_path,mysql_base,mysql_home,extra_params)
    # 3. 初始化数据库
    install_default_database(host,user,password,data_path,mysql_base,port,mysql_home)
